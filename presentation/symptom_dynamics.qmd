...
institute: "May 3, 2024"  #change date
author: "Kyuri Park"
format: 
  revealjs:
...
Mental disorders stem from a single common underlying cause for their various symptoms, following the physical disease model [@hyland2011origins].

::: r-stack
![](img/commoncause1.png){.absolute top="200" left="200" width="60%"} ![](img/commoncause2.png){.fragment .absolute top="200" left="200" width="60%"}
:::
...
<!-- ::: -->
::: notes
So at the very start, the traditional view on mental disorder was this common cause model, which says mental disorder stems from this single underlying cause, here let's say depression.
So this depression as a latent entity, it causes all the observable symptoms, like this; so this model basically follows from this physical disease model where you have for example, a cancer cell causes various symptoms like pain, headache, dizziness and etc., and you know if you remove the root cause, the cancer, then the symptoms again all go away.
But is that also the case in depression?
Does it really make sense to think of depression as a single entity that we can eliminate to cure the symptoms?
:::
...
::: notes
And a lot of people start thinking it does not make sense.
And one of the first persons who pointed out, was Denny Borsboom, and he says we should look at it as a network of symptoms.
...
<!-- So in this study, with our model that I am going to show, we are going to assess whether we can replicate these features of depression dynamics, except for bistability. --> <!-- For bistability, we would more or less presume that our system has bistability and we formalize our model such that it satisfies this condition. --> <!-- For the rest, we will see if our model can retrieve such features through our simulation. --> <!-- For example, for resilience, we will specify different levels of resilience within our model and examine whether the resulting statistical network models exhibit different structures. --> <!-- So this is sort of top-down approach, evaluating whether the proposed features of depression dynamics are supported within our modeling framework. -->
:::
...
I didn't really have any data back then, so I relied on insights from some empirical studies and mainly common sense to determine the weights and causal directions, which results in this adjacency matrix, A.
So it is literally a toy model.
...
The first top expression shows the ODE, defining the interaction between the symptoms.
...
$\alpha_{ii}$ is the diagonal of the matrix A, the self-reinforcing loop.
...
And here we define $f$ to be a quadratic function, because it seems to be one of the simplest functions I could think of for a monotonic non-linear increasing function, but it was my arbitrary choice.
So in the end what it says is that, as the $S_i$ level increases, the effects of other symptoms are amplified by $\delta_i \cdot S_i^2$.
...
But the question is then, how we defined the beta.
...
Then, when we give the shock to system, we raise $\beta_i$ like this, closer to the second critical point, where depressed state is the only stable equilibrium.
So the symptoms will likely go towards the depressed regime.
...
And the reason why we did this is, to make it align with those empirical observations where the suicidal symptom exhibits much lower activation compared to the other symptoms, probably because of its extreme nature.
okay?
:::
...
::: notes
So the results.
Quickly about the set up, so for each iteration, we simulated over 4000 timepoints, and the shock was given at where t was equal to 1000 and the shock lasted for 500 time points.
And for each scenario, we repeated the simulation for 500 times.
Alright?
...
<!-- Also, there is noticeable fluctuation occurring in the guilty symptom, which is due to its narrow bistable range that makes it prone to flipping with small perturbations. --> <!-- This fluctuation also affects concentration, because concentration is mainly influenced by guilty. --> <!-- Lastly, motor does not show much of hysteresis, and the reason is that motor is mainly affected by suicidal, which does barely gets activated in this case. -->
...
(SCROLL DOWN) And the aggregated symptom level also exhibits reduced hysteresis, as the system recovers before where t was 2000, so shortly after the shock is removed.
...
So in summary, we did find some clear hysteresis effects in our model, and also variations in network connectivity across different time phases as well as different resilience level.
<!-- And just to point out, these various networks are generated based on the same mechanistic model. -->
:::
...
So this shows distinctive network structures in different level of resilience.
:::
...
::: r-stack
![](img/totavgnetwork.png){.absolute top="120" left="1" width="500"} [simulated network]{.absolute bottom="40" left="150" style="font-size: 0.7em;"}
...
![](img/totavgnetwork_col.png){.fragment .absolute top="126" left="0" fragment-index="2" width="502"}
:::
...
Let me remind you the toy model again.
...
Also, the network shows a clear clustering structure: most nodes on the upper-left side of the simulated network are part of cycles, while the ones on the bottom-right  are not part of cycles.
(CLICK).
...
-   Comparing the [HELIUS](https://www.heliusstudy.nl/nl/researchers/the-helius-study/) data network.

![](img/boot_centrality_SD.png){width="40%"}

![](img/boot_edgePlot_SD.png)

-   Conducting robustness checks across various parameter values.

![](img/betaplot.png){width="70%"}
![](img/sigmaplot.png){width="70%"}
![](img/interactionplot.png){width="70%"}

::: notes
So that's what I basically found so far through this study.. Then, what I am currently doing is, well now that I've got hands on this HELIUS data, I've been trying to compare our model to this dataset.
So here i have some fresh-off plots that I got just yesterday. And mind you, they are still running as I speak at this moment, so these are all preliminary results...

Just to show you some of'em; well this is where I tried to compare the centrality of our simulated network with the HELIUS data network. And what i did was that, I bootstrapped the data, so these are results from 1000 bootstrapped samples for both. 
But It didn't really spice things up much. You see, with sample sizes as big as they are — we're talking about 24,000 samples for the HELIUS data and over 300,000 samples for my simulated data — the variations were barely there; they are super stable so you see very small standard deviations here. Anyways, the cool thing is, the resulting centrality didn't stray too far from each other, which I thought was pretty neat. Considering that I went into this completely blind, I had zero idea about this HELIUS dataset, so getting it this close for me was a pretty good surprise.

And here I am also comparing the edge weights of data network and our simulated network. So these are all the possible edge sets given my 9 symptom variables. Some of them have quite different values. Like, guilty-suicide, guilty-concentration, and anhedonia-sleep. And these deviations were also reflected in the strength centrality plot. So, those are the nodes that stand out in terms of centrality differences.

And, on top of all that, I've been running some robust checks on our results as well. You see, I kinda pulled those parameter values out of thin air, so now I'm putting them to the test to see if our main findings still hold. I'm tweaking parameters like beta and sigma to see if the differences in network density across different resilience levels are robust against different values of parameters. It's a bit of work in progress, but that's basically where I'm at right now.
:::
...
<!-- ![](img/palmer_prediction.png){.absolute .fragment .fade-in-then-semi-out top="150" right="50" width="400" fragment-index="2"}  -->
...
And how does our model differs or what does this model actually adds to the existing ones?
...
And secondly, our model differs in its continuous nature, which allows us to simulate symptom trajectories continuously rather than getting binary responses, such as in the Ising model, which is one of the popular models currently being used in this field.

<!-- And this continuity can actually provide a significant advantage. -->
<!-- Lourens introduced me to this work from Palmer, who mainly works in climate and weather prediction. -->
<!-- The core concept of Palmer's work revolves around the idea that with sufficient information about the initial state, we can potentially predict the continuous trajectory of a dynamical system. -->

<!-- In this case, they were predicting temperature using what they call an ensemble prediction model. -->
<!-- This model essentially adds perturbations to the initial conditions and generate trajectories based on stochastic differential equations. -->
<!-- In the graph, you see the dashed line, that is the actual temperature, and the gray lines represent trajectories from the model. -->
<!-- And here, their predictions were quite okay, as the real temperature fell within the predicted bounds. -->
...
Also, about the feedback loops and causal directions of the model.
It is one of the hot topics, causality and causal discovery.
I am also very much interested in this.
The thing is there are some promising causal algorithms that could learn the causal structure even with the feedback loops present in the model.
So hopefully with the data I have now, I would like to delve deeper into this.
...
I'd like to thank to my supervisor, Dr. Vitor Vasconcelos who has been guiding me through this work. And also to my potential promotor Mike Lees.
Alright, thanks a lot again for your attention.
:::
