...
Because in this case, the symptom activation does not easily propagate through the network so hysteresis therefore can also be hardly observed.
...
And as $\beta$ surpasses this first critical point, both 0 and 1 become stable states, together with an unstable state in between, (CLICK) which then can be represented by this smooth W shape bi-stable landscape.
And as $\beta_i$ exceeding $\omega_i$, the second critical point, 0 transitions to an unstable state, leaving 1 as the only stable equilibrium (CLICK), as you can see.
...
So again this shows distinctive network structures in different level of resilience. 
:::
...
I would say first, our model differs in its continuous nature, which allows us to simulate symptom trajectories continuously rather than getting binary responses, such as in the Ising model. Also, we've incorporated the non-linearity term that can be controlled by the delta parameter, which makes our model more flexible by being able to capture the nonlinear responses in the dynamics.

But of course, this is just a very start.
There are much more could be done in the future.
For instance, we would actually like to investigate further on this non-linearity aspect in the dynamics, as we for now just used a very simple expression, but it probably is not as simple as how we defined in reality.
...
The thing is there are some promising causal algorithms that could learmn the causal structure even with the feedback loops present in the model. So hopefully with the data I have now, I would like to delve deeper into this. If anyone working on causal discovery / causal inference, I would like to hear about what you think about this as well.
